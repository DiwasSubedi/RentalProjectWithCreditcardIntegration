...
public class UserController {
	...
	public String continueCheckout(@PathVariable("id") Integer id, Model model) {
		...
		CompositeCarModel.getReservationInstance().setCar(car);					
	...
	public String continueCheckout(Model model,@PathVariable("id") Integer id) {
				...
				List<Reservation> listReservation = resser.getAllReservation();
				for(Reservation res: listReservation){
					if(res.getDropoffdate().equals(CompositeCarModel.getReservationInstance().getDropoffdate())&&res.getPickupdate().equals(CompositeCarModel.getReservationInstance().getPickupdate())
							&&res.getDropofflocation().equals(CompositeCarModel.getReservationInstance().getDropofflocation())&&res.getPickuplocation().equals(CompositeCarModel.getReservationInstance().getPickuplocation())){
						
						List<Reservation> list=selectedinsurance.getInsListRese();
						list.add(res);
						selectedinsurance.setInsListRese(list);
					}
				}
				System.out.println("the Insurance we are updating is :" +selectedinsurance.getId());
				System.out.println("the number of reservation for this insurance is "+ selectedinsurance.getInsListRese().size());
				//insser.delete(selectedinsurance.getId());
				//insser.save(selectedinsurance);
				
				
				model.addAttribute("carwithinsurance",CompositeCarModel.getCarInstance());
				model.addAttribute("insurance",selectedinsurance);
		        ...
		        cal.set(Calendar.MONTH, Integer.parseInt(splittedDate[0]));
		        cal.set(Calendar.DAY_OF_MONTH, Integer.parseInt(splittedDate[1]));
		        
		    	String[] splittedDate1=CompositeCarModel.getReservationInstance().dropoffdate.split("/");
				System.out.println(splittedDate1[0]);
				Calendar cal1 = Calendar.getInstance();
				cal1.set(Calendar.YEAR, Integer.parseInt(splittedDate1[2]));
				cal1.set(Calendar.MONTH, Integer.parseInt(splittedDate1[0]));
				cal1.set(Calendar.DAY_OF_MONTH, Integer.parseInt(splittedDate1[1]));
				...
				int totalWithInsurance = selectedinsurance.getCostPerDay()*totaldays;
